export const STUDY_DATA = [
  {
    id: 'cloud', icon: '☁️', title: 'Cloud Fundamentals', badge: 'Foundation',
    concepts: [
      '<strong>6 Advantages of Cloud</strong>: Trade CAPEX→OPEX, Economies of Scale, Stop Guessing Capacity, Speed & Agility, Stop Running Data Centers, Go Global in Minutes',
      ...
      '<strong>Agility</strong> = deploy/experiment in minutes instead of weeks → reduces time-to-market significantly',
      '<strong>Economies of Scale</strong>: AWS buys hardware in massive bulk → passes savings to customers as lower prices over time'
    ],
    traps: [
      'Elasticity ≠ Scalability. Elasticity = dynamic short-term auto-scaling. Scalability = planned long-term capacity growth',
      'Agility = speed to deploy resources, NOT automatic scaling (that is Elasticity)',
      'Cloud does NOT eliminate ALL costs — OPEX still exists (pay per use). It eliminates CAPEX (upfront)',
  ...
  {
    id: 'infra', icon: '🌍', title: 'Global Infrastructure', badge: 'Regions/AZs',
    concepts: [
      ...
      '<strong>AZ interconnect</strong>: Connected via <2ms high-bandwidth private fiber → enables Synchronous Replication between AZs',
    ...
    traps: [
      'Edge Locations ≠ Availability Zones. Edge = CDN cache for CloudFront. AZ = full compute + storage data center',
      'AZ ≠ a single building. An AZ can comprise multiple buildings within the same fault isolation boundary',
  ...
  {
    id: 'service-models', icon: '🧱', title: 'Service Models', badge: 'IaaS/PaaS/SaaS',
    concepts: [
      '<strong>On-Premises</strong>: YOU own and manage EVERYTHING — rack, network, OS, runtime, apps, data',
      ...
      '<strong>Responsibility boundary</strong>: Shifts upward (toward app layer) as you move IaaS → PaaS → SaaS',
      '<strong>Elastic Beanstalk</strong>: PaaS — auto-provisions EC2, ASG, ELB, RDS from app code. You still can SSH into EC2',
      '<strong>Lambda execution model</strong>: Event → Lambda invoked → code runs → billed per ms + invocation count'
    ],
    traps: [
      'IaaS = EC2 (NOT PaaS). PaaS = Elastic Beanstalk, RDS. SaaS = WorkMail, Chime',
      'In SaaS, customer still owns their DATA — AWS doesn\'t own your business data',
      'Lambda is FaaS (subset of Serverless/PaaS), NOT SaaS. You still write and own the code',
      'Elastic Beanstalk is PaaS — it provisions EC2 under the hood. Guest OS patches = AWS responsibility here'
    ],
    related: ['cloud', 'shared-responsibility', 'ec2', 'containers', 'deployment']
  },
  {
    id: 'deployment', icon: '🏗️', title: 'Deployment Models', badge: 'Cloud/Hybrid/On-Prem',
    concepts: [
      ...
      '<strong>Cloud-Native</strong>: Architectures fully designed for cloud — microservices, auto-scaling, managed services, event-driven',
      ...
      '<strong>AWS Outposts</strong>: Hybrid implementation — AWS manages the rack on-premises, you consume AWS services locally'
    ],
    traps: [
      'Hybrid ≠ just using both cloud and on-prem independently. They must INTEGRATE and communicate as one architecture',
  ...
  {
    id: 'shared-responsibility', icon: '🤝', title: 'Shared Responsibility', badge: 'Security',
    concepts: [
      ...
      '<strong>Shared domain — Encryption</strong>: Customer CONFIGURES encryption settings. AWS provides KMS Key Management infrastructure',
    ...
    traps: [
      ...
      'In PaaS/SaaS, OS patching shifts to AWS — but DATA SECURITY always remains customer\'s responsibility',
  ...
  {
    id: 'ec2', icon: '💻', title: 'Amazon EC2', badge: 'IaaS Compute',
    concepts: [
    ...
    traps: [
      'EC2 is NOT serverless — you manage the OS, and you pay even when the instance is idle',
      'Spot instances = cheapest but CAN BE INTERRUPTED. Never use for uninterruptible critical real-time workloads',
      'Reserved ≠ Savings Plans. RI = specific instance type + region binding. Savings Plans = flexible compute spend commitment',
      'EBS volume is AZ-scoped — you CANNOT attach an EBS from AZ-1a to an EC2 in AZ-1b'
    ],
    related: ['infra', 'service-models', 'storage', 'architecture', 'scaling', 'shared-responsibility']
  },
  {
    id: 'storage', icon: '🗄️', title: 'Storage Services', badge: 'S3/EBS/EFS',
    concepts: [
      ...
      '<strong>Instance Store</strong>: Physically attached to the host server. Highest I/O speed. EPHEMERAL — data LOST on stop/terminate',
    ...
    traps: [
      'S3 is NOT a filesystem — you cannot mount it as a drive. Use EFS for shared file system access from EC2',
      'EBS = Zonal resource. Cannot span AZs — if the AZ fails and EBS is not backed up, data is at risk',
      'Instance Store data is LOST on instance stop (and terminate) — NEVER store important data here without backup',
  ...
  {
    id: 'architecture', icon: '🛡️', title: 'Architecture & DR', badge: 'HA/DR/Resilience',
    concepts: [
      ...
      '<strong>DR Ladder 1 — Backup & Restore</strong>: Cheapest. Hours-level RTO/RPO. Cold data in S3 Glacier triggered after disaster',
      '<strong>DR Ladder 2 — Pilot Light</strong>: Only CORE services (DB) always running. Everything else is off. Scale up after disaster. Minutes RTO',
      '<strong>DR Ladder 3 — Warm Standby</strong>: Scaled-DOWN but fully functional copy running. Faster failover. More cost',
      '<strong>DR Ladder 4 — Active-Active (Multi-Site)</strong>: Zero downtime. Two full equal environments. Highest cost and complexity',
    ...
    traps: [
      ...
      'Pilot Light ≠ Warm Standby. Pilot Light = DB only running. Warm Standby = full env at reduced capacity',
      'Active-Active = lowest RTO/RPO possible but HIGHEST cost and operational complexity'
    ],
    related: ['infra', 'ec2', 'storage', 'scaling', 'cloud']
  },
  {
    id: 'scaling', icon: '⚖️', title: 'Load Balancing & Auto Scaling', badge: 'ELB/ASG',
    concepts: [
      ...
      '<strong>ALB (Application LB — Layer 7)</strong>: HTTP/HTTPS only. Path-based (/api → service A), host-based, header-based routing',
      '<strong>NLB (Network LB — Layer 4)</strong>: TCP/UDP. Millions req/sec. Ultra-low latency. Supports static IP and Elastic IP',
      '<strong>GLB (Gateway LB — Layer 3)</strong>: Routes traffic through security virtual appliances (firewalls, IDS/IPS) transparently',
    ...
    traps: [
      ...
      'Auto Scaling alone does NOT guarantee HA — you must deploy ASG ACROSS multiple AZs',
  ...
  {
    id: 'containers', icon: '🐳', title: 'Containers & Serverless', badge: 'Lambda/ECS/EKS',
    concepts: [
    ...
    traps: [
      ...
      'Lambda has 15-minute MAX execution limit — not suitable for long-running batch jobs or persistent connections',
      'EKS ≠ ECS. EKS uses Kubernetes (open-source). ECS uses AWS proprietary orchestration. Both run containers',
...
export const TOPIC_CONNECTIONS = {
  ...
  'ec2':                { connects: ['infra','service-models','storage','architecture','scaling','shared-responsibility'],
                         reason: 'EC2 is the core IaaS compute unit — it connects to AZ placement, storage attachment, scaling groups, and security patching' },
